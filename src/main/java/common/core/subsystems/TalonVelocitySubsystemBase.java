...
public class TalonVelocitySubsystemBase extends NAR_TalonPIDSubsystem implements NAR_Subsystem {
    ...
    public Command toVelocity(double velocity) {
        return Commands.runOnce(() -> motor.setControl(m_request.withVelocity(velocity)));
    }

    public Command toVelocity(DoubleSupplier velocity) {
        return Commands.runOnce(() -> motor.setControl(m_request.withVelocity(velocity.getAsDouble())));
    }
